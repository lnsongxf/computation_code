   ...
   if strncmp(myppual_flag,'cons',4)
       ...
       if strcmp(myppual_flag,'cons_eval_cell') % needs a partial construction and evaluation
           if isequal(mx,1) % combined full 1-d construction and evaluation
               ...
               coefs_MKL = myppualMKL_CMEX(MKL_flag_cons,breaks_MKL,Values_MKL,pieces_MKL,order_MKL,dim_MKL,Method,reshape(x{1},1,[]),left,indexDim_MKL,index_x_MKL);
               [v,index_x] = myppualMKL_CMEX(MKL_flag,breaks_MKL,coefs_MKL,pieces_MKL,order_MKL,dim_MKL,Method,reshape(x{1},1,[]),left,indexDim_MKL,index_x_MKL);
               index_x = {index_x};
           else  % combined partial construction and evaluation
               [v,index_x] = myppualMKL_CMEX(MKL_flag,breaks_MKL,Values_MKL,pieces_MKL,order_MKL,dim_MKL,Method,x,left,indexDim_MKL,index_x_MKL);
           end
       else % need a full construction first           
           % first construct the coefs
           MKL_flag_cons = -MKL_flag;
           coefs_MKL = myppualMKL_CMEX(MKL_flag_cons,breaks_MKL,Values_MKL,pieces_MKL,order_MKL,dim_MKL,Method,x,left,indexDim_MKL,index_x_MKL);
           % the final step
           ...
           elseif strcmp(myppual_flag,'cons_eval_matrix')
               [v,index_x] = myppualMKL_CMEX(MKL_flag,breaks_MKL,coefs_MKL,pieces_MKL,order_MKL,dim_MKL,Method,x,left,indexDim_MKL,index_x_MKL);
           end
       end
   elseif strcmp(myppual_flag,'eval_matrix_general')
